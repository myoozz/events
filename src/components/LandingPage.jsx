...
const C = {
  ...
  // Bug 7 — dark navy for sections that must not be pure black
  navy:'#16203A',
}
...
function ProductSim() {
  ...
  return (
    <div style={{ border:`1px solid ${C.border2}`, borderRadius:'16px', overflow:'hidden', background:C.white, boxShadow:'0 8px 40px rgba(26,25,21,0.08)' }}>
      <div style={{ padding:'16px 20px', background:C.bg, borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:'8px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'200px 1fr', minHeight:'340px' }}>
        <div style={{ background:C.bg2, borderRight:`1px solid ${C.border}`, padding:'16px 0' }}>
          {STEPS.map((s,i)=>(
            <div key={i} onClick={()=>setStep(i)} style={{ padding:'10px 20px', fontSize:'13px', cursor:'pointer', color:i===step?C.red:C.text2, background:i===step?C.redDim:'transparent', fontWeight:i===step?600:400, fontFamily:"'DM Sans',sans-serif", borderLeft:i===step?`3px solid ${C.red}`:'3px solid transparent', transition:'all 0.15s' }}>
              {s}
            </div>
          ))}
        </div>
        <div style={{ padding:'24px' }}>
          {step===0&&(
            <div>
              ...
              {[['Event name',"Your Client's product launch"],['Client','Your client · Consumer brand'],['Cities','Delhi · Mumbai'],['Date','15 Mar · 22 Mar'],['Agency fee','15%'],['GST','18%']].map(([l,v],i)=>(
                <div key={i} style={{ display:'flex', gap:'16px', marginBottom:'10px', fontSize:'13px' }}>
                  <span style={{ color:C.text3, width:'100px', flexShrink:0 }}>{l}</span>
          ...
          {step===1&&(
            <div>
              ...
              {[['Stage 40×20ft','40×20','ft',1,2],['LED Wall P3.9','10×6','ft',2,2],['Arch Gate','9×16','ft',1,1],['Registration Desk','8×3','ft',3,1]].map(([n,s,u,q,d],i)=>(
                <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 80px 40px 30px 30px', gap:'8px', padding:'7px 0', borderBottom:`1px solid ${C.border}`, fontSize:'12px', alignItems:'center' }}>
                  <span style={{ color:C.text, fontWeight:500 }}>{n}</span>
                  <span style={{ color:C.text3 }}>{s} {u}</span>
                  <span style={{ color:C.text3, textAlign:'center' }}>×{q}</span>
                  <span style={{ color:C.text3, textAlign:'center' }}>{d}d</span>
          ...
          {step===2&&(
            <div>
              ...
              {[['Production','₹8,40,000'],['Sound & Light','₹3,20,000'],['Branding','₹1,85,000'],['Manpower','₹72,000']].map(([c,a],i)=>(
                <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom:`1px solid ${C.border}`, fontSize:'13px' }}>
                  ...
                  <span style={{ color:C.text, fontWeight:600, fontFamily:"'Cormorant Garamond',serif", fontSize:'15px' }}>{a}</span>
                </div>
              ))}
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"'DM Sans',sans-serif", overflowX:'hidden' }}>
      ...
      <section id="forwho" style={{ padding:'100px 24px', background:C.white, borderTop:`1px solid ${C.border}` }}>
        <div style={{ maxWidth:'1060px', margin:'0 auto' }}>
          ...
          <R>
            {/* Bug 7 fix — was C.text (#1A1917 pure black), now C.navy (#16203A) */}
            <div style={{ padding:'36px 40px', background:C.navy, borderRadius:'16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'40px', alignItems:'center' }}>
              <div>
                ...
                {/* Bug 6 fix — text was "Professionals who switch quote faster" → corrected */}
      ...
      {/* STATS — Bug 7 fix: was C.text (#1A1917), now C.navy (#16203A) */}
      <div style={{ background:C.navy, borderTop:`1px solid ${C.border}`, padding:'56px 24px' }}>
